/**
 * Adapter boundary to upstream sysml-2ls validation.
 *
 * The public API sends service-owned request DTOs into this module. The module
 * creates a fresh request-local Langium workspace, builds upstream documents,
 * normalizes diagnostics, and returns only service-owned response DTOs.
 *
 * @example
 * ```ts
 ...
 *
 * @param request - Parsed validate request.
 * @returns Service-owned validation response.
 *
 * @example
 * ```ts
 * await validateSysML({
...
export async function validateSysML(
  ...
  const documents: Array<LangiumDocument<ast.Namespace>> = request.files.map(
    (file, index) => {
